...
Applies to every project and every session unless a project's own CLAUDE.md overrides it.
...
Machine specifics, hardware, accounts and personal defaults live in `profile.md`, imported below.
It is gitignored — this repo is public, and the profile is per-operator and per-machine.
Create it from `profile.md.example` (the bootstrap script does this for you).
